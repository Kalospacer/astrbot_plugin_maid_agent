...
通知链路：主 Agent 转述指令从 system_prompt 挪进本轮正文。

### 修复

- **管家任务通知不再向主 Agent 的 system_prompt 注入硬编码英文指令**。system_prompt 是人格区，由宿主拼接所选人格；插件往里写一次性任务指令属于污染人设。转述要求（用自己口吻简短转述、不复述中间过程、正文自动送达、仅媒体才调发送工具）改为写在本轮 `req.prompt` 的通知正文之前——随轮生效、不落会话历史。附带删掉原指令里「女仆已向用户播报中间步骤」的错误表述：默认 `show_maid_speech=false`，用户并没有看到过中间步骤。
