...
- **管家（子代理 SubAgent）**：接管全部工具调用能力（Shell、Python、文件、浏览器等），在后台执行任务，过程中的正文以 `管家名: ...` 直接说给用户听，最终汇报交给大小姐转述。管家的执行记录独立存放，不写进大小姐的对话历史。
...
管家干活过程中每一段自然语言正文都会以 `管家名: ...` 的形式直接发到聊天里，用户能看到它在想什么、做到哪一步，而不是干等一个黑箱。

最后一段正文**不发**——那是任务汇报，会随终态回灌给大小姐，由她用自己的话转述。这样同一份结论不会出现两遍。
